...
function scoreColors(score: number) {
  if (score >= 90) return { text: "text-emerald-600 dark:text-emerald-400", stroke: "#059669" };
  if (score >= 70) return { text: "text-amber-600 dark:text-amber-400", stroke: "#d97706" };
  return { text: "text-red-600 dark:text-red-400", stroke: "#dc2626" };
}
      ...
      <div className="card p-6 flex items-center gap-4">
        ...
        <div className="flex-1 space-y-3">
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500">
            Scorecard appears when the agents converge.
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex flex-wrap items-center gap-8">
        ...
        <div className="flex-1 min-w-[240px] space-y-4">
          <div className="grid grid-cols-3 gap-3 text-center">
            {[
              { n: sc.auto_verified, label: "auto-verified", cls: "chip-emerald" },
              { n: sc.review_needed, label: "needs review", cls: "chip-amber" },
              { n: sc.hard_fail, label: "hard fail", cls: "chip-red" },
            ].map(({ n, label, cls }) => (
          ...
          {sc.summary && (
            <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed
                          border-t border-slate-100 dark:border-slate-800 pt-3.5">
              {sc.summary}
            </p>
          )}
